...
function buildStream(options, WebSocket, Proxy) {
    ...
    WebSocket.onopen = () => {
        ...
        // Timeout 60s n?u kh�ng nh?n du?c response 
      ...
      switch (delta.type) {
        case 'change_thread_quick_reaction': {
          try {
            ...
            let userID = delta.messageMetadata?.actorFbId;
            let emoji = delta.untypedData?.thread_quick_reaction_emoji || delta.untypedData?.thread_icon;
    
            if (threadID && userID && emoji) {
              console.log(`[EMOJI-TRAP] Ph�t hi?n d?i emoji nhanh: Thread=${threadID}, User=${userID}, Emoji=${emoji}`);
              globalCallback(null, {
                type: "event",
                threadID: threadID.toString(),
                author: userID.toString(),
                logMessageType: "log:thread-icon",
                logMessageData: { thread_icon: emoji }
              });
            }
          } catch (err) {
            console.error(`[EMOJI-TRAP] L?i x? l� quick reaction:`, err);
          }
          break;
        }
        ...
        default: {
          // Ngo�i c�c type tr�n, n?u adminText c� ch? "emoji" ho?c "bi?u tu?ng c?m x�c", cung trap emoji
          try {
            if (delta.adminText && (
              delta.adminText.toLowerCase().includes('emoji') ||
              delta.adminText.toLowerCase().includes('bi?u tu?ng c?m x�c') ||
              delta.adminText.toLowerCase().includes('d� thay d?i bi?u tu?ng')
            )) {
              let threadID = delta.messageMetadata?.threadKey?.threadFbId || delta.messageMetadata?.threadKey?.otherUserFbId;
              let userID = delta.messageMetadata?.actorFbId || delta.actorFbId;
              let icon = "??"; // default
    
              const matchEmoji = delta.adminText.match(/d� thay d?i bi?u tu?ng c?m x�c nh�m th�nh (.+?)\./) ||
                                 delta.adminText.match(/changed the group emoji to (.+?)\./) ||
                                 delta.adminText.match(/d� thay d?i bi?u tu?ng th�nh (.+?)\./);
    
              if (matchEmoji?.[1]) icon = matchEmoji[1].trim();
    
              if (threadID && userID) {
                console.log(`[EMOJI-TRAP] Ph�t hi?n d?i emoji qua adminText: Thread=${threadID}, User=${userID}, Emoji=${icon}`);
                globalCallback(null, {
                  type: "event",
                  threadID: threadID.toString(),
                  author: userID.toString(),
                  logMessageType: "log:thread-icon",
                  logMessageData: { thread_icon: icon }
                });
              }
            }
          } catch (err) {
            console.error(`[EMOJI-TRAP] L?i x? l� adminText emoji:`, err);
          }
          break;
        }
      }
...
module.exports = function(defaultFuncs, api, ctx) {
  ...
  function restartMqtt(api, callback) {
    // Reset MQTT-related context and restart listening
    ctx.t_mqttCalled = false;
    ctx.syncToken = undefined;
    listenMqtt(defaultFuncs, api, ctx, callback);
  }
  ...
  return function(callback) {
    ...
    globalCallback = (callback || function(error, message) {
      if (error) {
        ...
        // Uu ti�n ki?m tra c?nh b�o scraping tru?c
          ...
          defaultFuncs
            ...
            .then((response) => {
              try {
                ...
                if (res.errors) {
                  return log.error("FBScrapingWarning", "Kh�ng th? x�a c?nh b�o Facebook.");
                }

                if (res?.data?.fb_scraping_warning_clear?.success) {
                  log.info("FBScrapingWarning", "�� x�a c?nh b�o Facebook th�nh c�ng. Kh?i d?ng l?i MQTT sau 1 gi�y...");
                  setTimeout(() => restartMqtt(api, globalCallback), 1000);
                }
              } catch (err) {
                log.error("FBScrapingWarning", "L?i ph�n t�ch JSON: " + err.message);
              }
            })
            .catch((err) => {
              log.error("FBScrapingWarning", "L?i g?i HTTP POST: " + err.message);
            });
        } else if (errorStr.includes("Not logged in") || !ctx.loggedIn) {
          // Ki?m tra l?i "Not logged in" sau khi d� x? l� c?nh b�o
          log.error("LoginError", "Phi�n dang nh?p kh�ng h?p l? ho?c d� h?t h?n.");
          return msgEmitter.emit("error", { error: "Not logged in" });
        } else {
          // C�c l?i kh�c du?c ph�t ra nhu l?i chung
    ...
    // Reset m?t s? tr?ng th�i
    if (!ctx.firstListen) ctx.lastSeqId = null;
    ctx.syncToken = undefined;
    ctx.t_mqttCalled = false;

    // Y�u c?u tuong t? getThreadList
    form = {
      av: ctx.globalOptions.pageID,
      queries: JSON.stringify({
        o0: {
