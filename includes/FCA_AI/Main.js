...
function buildAPI(globalOptions, html, jar) {
    ...
    if (endpointMatch) {
        ...
        logger.Normal(`Khu vực MQTT: ${region}`);

        if (endpointMatch.input.includes("601051028565049")) {
            logger.Warning("Em đang vượt cảnh báo...");
    ...
    async function clearWarning(retries = 0) {
        try {
            ...
            if (data.data?.fb_scraping_warning_clear?.success) {
                logger.Normal("Em đã loại bỏ cảnh báo rồi nha");
                logger.Normal("Đợi em tí, đang khởi động lại ...");
