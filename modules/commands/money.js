module.exports.config = {
  ...
  module.exports.run = async function ({ Currencies, api, event, args, Users,permssion }) {
   ...
   return api.sendMessage(`Đã chuyển cho ${name} ${bet}$`,event.threadID)}
    ...
    if (money === Infinity) return api.sendMessage(`[ Ví Tiền Mặt ]\n────────────────────\n👤 Name: ${name}\n🎫 Số tiền: Vô hạn\n────────────────────\n✏️ Dùng work nếu muốn kiếm thêm thu nhập nhé!`,event.threadID)
    if (money === null) return api.sendMessage(`[ Ví Tiền Mặt ]\n────────────────────\n👤 Name: ${name}\n🎫 Số tiền: 0$\n────────────────────\n✏️ Dùng work nếu muốn kiếm thêm thu nhập nhé!`,event.threadID)
    if (!args[0] || !args[1]) return api.sendMessage(`[ Ví Tiền Mặt ]\n────────────────────\n👤 Name: ${name}\n🎫 Số tiền: ${money}$\n────────────────────\n✏️ Dùng work nếu muốn kiếm thêm thu nhập nhé!`,event.threadID)
  }