...
global.bypass = require("./includes/login/loginandby.js"); // Removed duplicate assignment
global.utils = require("./utils");
global.tokens = require("./includes/tokens.json")
...
// ====== Khởi tạo Biến Toàn Cục Đính Kèm (global.khanhdayr) ======
if (!global.khanhdayr || !Array.isArray(global.khanhdayr)) {
    global.khanhdayr = [];
    console.log("[INIT] Đã khởi tạo biến global.khanhdayr dưới dạng mảng rỗng.");
}
...
function onBot({ models }) {
    ...
    login(loginData, async (loginError, loginApiData) => {
        ...
        /* logger("AUTO check Rent đã hoạt động!", "[ RENT ]")
        ...
        function listenerCallback(error, message) {
            if (error) {
                ...
                if (errorString.includes("601051028565049")) {
                    const form = {
                    ...
                    loginApiData.httpPost(
                        ...
                        (e, i) => {
                            ...
                            try {
                                ...
                                return logger(
                                    `Lỗi khi parse response từ Facebook: ${parseError.message}`,
                                    "error"
                                );
                            }
                            ...
                            if (e || res.errors) {
                                return logger(
                                    `Lỗi không thể xóa cảnh cáo của Facebook: ${JSON.stringify(e || res.errors)}`,
                                    "error"
                                );
                            }
                    ...
                    logger(
                        global.getText("mirai", "handleListenError", errorString),
                        "error"
                    );
                    setTimeout(() => connect_mqtt(), 1000 * 60); // Retry after 1 minute
                }
                return;
            }
            if (
...
(async () => {
    try {
        ...
        logger(
            global.getText("mirai", "errorConnectDatabase", JSON.stringify(error)),
            "[ DATABASE ]"
        );
    }
})();
